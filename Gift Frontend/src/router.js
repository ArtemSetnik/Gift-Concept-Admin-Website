  ...
  routes: [

    {
      ...
      children: [
        ...
        },
        {
          path: '/admin/buyers',
          name: 'Buyers',
          component: () => import('./views/admin/Buyers.vue'),
          meta: {
            rule: 'admin'
          }
        },
        {
          ...
          name: 'Buyers',
          component: () => import('./views/admin/RegisterRequests.vue'),
          meta: {
            rule: 'admin'
          }
        },
        ...
        {
          path: '/admin/orders/buy',
          name: 'Buy Orders',
          component: () => import('./views/admin/BuyOrders.vue'),
          meta: {
            rule: 'admin'
          }
        },
        {
          path: '/admin/orders/drive',
          name: 'Drive Orders',
          component: () => import('./views/admin/DriveOrders.vue'),
          meta: {
            rule: 'admin'
          }
        },
        ...
        // =============================================================================
        {
          ...
          redirect: function ($router) {
            if (auth.isAdminAuthenticated()) {
              return "/admin";
            } else if (auth.isSellerAuthenticated()) {
              return "/seller";
            } else if (auth.isDriverAuthenticated()) {
              return "/driver";
            } else {
              return "/pages/home";
            }
    ...
    // =============================================================================
    {
      ...
      children: [
        ...
        {
          path: '/global/pending',
          name: 'Server is pending',
          component: () => import('./views/global/Pending.vue'),
          meta: {
            role: 'anonymos'
          }
        },
  ...
  firebase.auth().onAuthStateChanged(async () => {
    try {
      if (to.meta.rule == 'admin') {
        ...
        if (!await auth.isSetAdmin()) {
          if (to.path == '/admin/setup') {
            return next();
          }
          return router.push({ path: '/admin/setup' })
        }
      }
      if (to.meta.rule == 'seller') {
        if (auth.isSellerAuthenticated()) {
          return next();
        }
      }
      if (to.meta.rule == 'driver' && auth.isDriverAuthenticated()) {
        return next();
      }
      if (to.meta.rule == 'anonymous') {
        return next();
      }

      router.push({ path: '/pages/login', query: { to: to.path } })
    } catch (error) {
      router.push({ path: '/pages/error-500', query: { to: to.path, error } });
    }
